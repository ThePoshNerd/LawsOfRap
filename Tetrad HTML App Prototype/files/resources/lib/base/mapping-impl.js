function __init_mapping() {
    ...
    var UIHandler = window.__uihandler || (function () {
        ...
        UIHandler.prototype.hideTemplateComponents = function (mapping) {
            for (var i = 0, i_limit = mapping.directions.length; i < i_limit; i++) {
                ...
                if (direction.to_type === "UI") {
                    ...
                    for (var j = 0, j_limit = dynamicContainersIDs.length; j < j_limit; j++) {
                        $("#" + direction.to_name + " [dsid='" + allIDs[j] + "']").attr("data-appery-tpl", "true");
                    }

                    for (var j = 0, j_limit = allIDs.length; j < j_limit; j++) {
                        Apperyio.c15r($("#" + direction.to_name + " [dsid='" + allIDs[j] + "']"), "set", "visible", "false");
                    }
                }
            }
        };
        ...
        UIHandler.prototype.showGeneratedComponents = function (direction) {
            if (direction.to_type === "UI" && _.isArray(direction.mappings)) {
                _.chain(direction.mappings)
                    ...
                    .compact()
                    .each(function (e) {
                        Apperyio.c15r(e, "set", "visible", true);
                    });
        ...
        UIHandler.prototype.showStaticContainerComponents = function (direction) {
            if (direction.to_type === "UI" && _.isArray(direction.mappings)) {
                ...
                for (var j = 0, j_limit = staticContainersIDs.length; j < j_limit; j++) {
                    Apperyio.c15r($("#" + direction.to_name + " [dsid='" + staticContainersIDs[j] + "']"), "set", "visible", "true");
                }
            }
        };
        ...
        UIHandler.prototype.__set = function (context, expr, data, mod, transformation) {
            if (expr.length === 0 || !expr[0] || data === undefined) return;

            var b = expr[0].replace(/'/g, "").split(":"),
                dsid = b[0],
                attr = b[1],
                elementSelector = "[dsid='" + dsid + mod + "']",
                cloneSelector,
            ...
            if ($.isArray(data) && element.attr("data-appery-tpl") === "true") {
                for (var i = 0; i < data.length; i++) {
                    cloneSelector = "[dsid='" + dsid + mod + "_" + i + "']";
                    clone = $(cloneSelector, context);

                    if (clone.length === 0) {
                        clone = Apperyio.c15r(element, "clone", i).insertBefore(Apperyio.c15r(element, "wrapper"));
                        Apperyio.c15r(clone, "unwrapper").removeAttr("data-appery-tpl");
                    }

                    if (attr) {
                        dataBuffer = transformation.transform(data[i], clone);
                        Apperyio.c15r(Apperyio.c15r(clone, "unwrapper"), "set", attr, dataBuffer);
                    } else {
                        if (expr.length === 1) {
        ...
        UIHandler.prototype.set = function (expr, data, transformation) {
            this.__set($("#" + Apperyio.CurrentScreen), MEP.validateAndPreparePath(expr), data, "", transformation);
        };
