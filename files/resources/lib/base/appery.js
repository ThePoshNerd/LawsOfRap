(function ($) {
    ...
    Apperyio.DataSource = $a.createClass(null, {
            ...
            __successHandler: function (data, status, xhr) {
                var resultField = this.service instanceof Apperyio.RestService ? "body" : "data";
                ...
                if (_.isString(data) && (this.service.__requestOptions.dataType === "json"
                    ...
                    this.response[resultField] = JSON.parse(data);
                } else if (this.service.__requestOptions.dataType === "xml") {
                    this.response[resultField] = $.xml2json(data);
                } else {
    ...
    Apperyio.CameraService = $a.createClass(null, {
        ...
        getEncodingType: function (strType) {
            ...
            return ENCODING_TYPES[strType] || Camera.EncodingType.JPEG;
        },
        ...
        getDestinationType: function (strType) {
            ...
            return DESTINATION_TYPES[strType] || Camera.DestinationType.DATA_URL;
        },
        ...
        getSourceType: function (strType) {
            ...
            return SOURCE_TYPES[strType] || Camera.PictureSourceType.CAMERA;
        },
    ...
    Apperyio.refreshScreenFormElements = function (screen) {
        ...
        try {
            ...
            ctx.find("div.ui-select select").each(function () {
                var select = $(this),
                    placeholderOptions = select.find("option[data-placeholder='true']"),
                    realOptions = select.find("option[data-placeholder!='true']"),
                    selectedOptions = realOptions.filter(":selected");

                // We are changing selects with at least one real option only
                if (realOptions.length > 0) {
                    // Default template component text must be exactly empty string to have select menu title hidden
                    placeholderOptions.each(function () {
                        var option = $(this);
                        if (option.text().trim() === "") option.text("");
                    });

                    if (selectedOptions.length === 0) {
                        realOptions.eq(0).attr("selected", "selected");
                    }
                }
